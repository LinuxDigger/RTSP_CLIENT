...
BasicUDPSource* BasicUDPSource::createNew(UsageEnvironment& env,
	...
	return new BasicUDPSource(env, cpObj, inputGS);
}

BasicUDPSource::BasicUDPSource(UsageEnvironment& env, CommonPlay *cpObj,
		Groupsock* inputGS) :
		FramedSource(env, cpObj), fInputGS(inputGS), fHaveStartedReading(False) {
