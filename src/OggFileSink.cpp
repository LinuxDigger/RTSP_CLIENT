...
#include "MPEG2TransportStreamMultiplexor.h" // for calculateCRC()
#include "FramedSource.h"

OggFileSink* OggFileSink::createNew(UsageEnvironment& env, char const* fileName,
		unsigned samplingFrequency, char const* configStr, unsigned bufferSize,
	...
	do {
		...
		return new OggFileSink(env, fid, samplingFrequency, configStr,
				bufferSize, perFrameFileNamePrefix, cpObj);
	} while (0);
...
OggFileSink::OggFileSink(UsageEnvironment& env, FILE* fid,
		...
		FileSink(env, fid, bufferSize, perFrameFileNamePrefix, 0, cpObj), fSamplingFrequency(
				samplingFrequency), fConfigStr(configStr), fHaveWrittenFirstFrame(
		...
		double ptDiff =
				(presentationTime.tv_sec - fFirstPresentationTime.tv_sec)
						+ (presentationTime.tv_usec
								- fFirstPresentationTime.tv_usec) / 1000000.0;
	...
	if (!fHaveWrittenFirstFrame) {
		...
		if (fConfigStr != NULL && fConfigStr[0] != '\0') {
			...
			parseVorbisOrTheoraConfigStr(fConfigStr, identificationHdr,
					identificationHdrSize, commentHdr, commentHdrSize, setupHdr,
					setupHdrSize, identField);
			if (identificationHdrSize >= 42
					&& strncmp((const char*) &identificationHdr[1], "theora", 6)
							== 0) {
