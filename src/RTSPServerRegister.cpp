...
static void rtspRegisterResponseHandler(RTSPClient* rtspClient, int resultCode,
		char* resultString, CommonPlay *cpObj); // forward
...
public:
	RegisterRequestRecord(RTSPServer& ourServer, unsigned requestId,
			...
			RTSPRegisterSender(ourServer.envir(), remoteClientNameOrAddress,
					...
					0/*verbosityLevel*/,
#endif
					NULL), fOurServer(ourServer), fRequestId(requestId), fResponseHandler(
					responseHandler) {
...
unsigned RTSPServer::registerStream(ServerMediaSession* serverMediaSession,
	...
	new RegisterRequestRecord(*this, requestId, remoteClientNameOrAddress,
			remoteClientPortNum, rtspURL(serverMediaSession), responseHandler,
			authenticator, receiveOurStreamViaTCP, proxyURLSuffix);
...
static void rtspDeregisterResponseHandler(RTSPClient* rtspClient,
		int resultCode, char* resultString, CommonPlay *cpObj); // forward
...
static void rtspDeregisterResponseHandler(RTSPClient* rtspClient,
		int resultCode, char* resultString, CommonPlay *cpObj) {
...
RTSPServer::RTSPClientConnection::ParamsForREGISTER::ParamsForREGISTER(
		...
		char const* proxyURLSuffix) :
		fCmd(strDup(cmd)), fOurConnection(ourConnection), fURL(strDup(url)), fURLSuffix(
...
void RTSPServer::RTSPClientConnection::handleCmd_REGISTER(
	...
	if (fOurRTSPServer.weImplementREGISTER(cmd, proxyURLSuffix, responseStr)) {
		...
		ParamsForREGISTER* registerParams = new ParamsForREGISTER(cmd, this,
				url, urlSuffix, reuseConnection, deliverViaTCP, proxyURLSuffix);
		envir().taskScheduler().scheduleDelayedTask(0,
				(TaskFunc*) continueHandlingREGISTER, registerParams, NULL);
...
void parseTransportHeaderForREGISTER(char const* buf, Boolean &reuseConnection,
		Boolean& deliverViaTCP, char*& proxyURLSuffix) {
...
void RTSPServer::RTSPClientConnection::continueHandlingREGISTER1(
	...
	ourServer->implementCmd_REGISTER(params->fCmd, params->fURL,
			params->fURLSuffix, socketNumToBackEndServer,
...
RTSPServerWithREGISTERProxying::RTSPServerWithREGISTERProxying(
		...
		unsigned reclamationSeconds, Boolean streamRTPOverTCP,
		int verbosityLevelForProxying) :
		RTSPServer(env, ourSocket, ourPort, authDatabase, reclamationSeconds), fStreamRTPOverTCP(
				streamRTPOverTCP), fVerbosityLevelForProxying(
				verbosityLevelForProxying), fRegisteredProxyCounter(0), fAllowedCommandNames(
		NULL), fAuthDBForREGISTER(authDatabaseForREGISTER) {
}
...
void RTSPServerWithREGISTERProxying::implementCmd_REGISTER(
	...
	if (strcmp(cmd, "REGISTER") == 0) {
		...
		ServerMediaSession* sms = ProxyServerMediaSession::createNew(envir(),
		NULL, this, url, proxyStreamName, NULL, NULL, tunnelOverHTTPPortNum,
...
UserAuthenticationDatabase* RTSPServerWithREGISTERProxying::getAuthenticationDatabaseForCommand(
		char const* cmdName) {
