...
H265VideoRTPSink* H265VideoRTPSink::createNew(UsageEnvironment& env,
	...
	H265VideoRTPSink* result = new H265VideoRTPSink(env, RTPgs,
			rtpPayloadFormat, vps, vpsSize, sps, spsSize, pps, ppsSize);
...
char const* H265VideoRTPSink::auxSDPLine() {
	...
	char const* fmtpFmt = "a=fmtp:%d profile-space=%u"
			...
			";sprop-sps=%s"
			";sprop-pps=%s\r\n";
	unsigned fmtpFmtSize = strlen(fmtpFmt) + 3 /* max num chars: rtpPayloadType */
	+ 20 /* max num chars: profile_space */
	+ 20 /* max num chars: profile_id */
	+ 20 /* max num chars: tier_flag */
	+ 20 /* max num chars: level_id */
	+ strlen(interopConstraintsStr) + strlen(sprop_vps) + strlen(sprop_sps)
			+ strlen(sprop_pps);
