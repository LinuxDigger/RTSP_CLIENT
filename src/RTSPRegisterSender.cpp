...
#include "strDup.h"
#include <GroupsockHelper.h> // for MAKE_SOCKADDR_IN
...
RTSPRegisterOrDeregisterSender::RTSPRegisterOrDeregisterSender(
	...
	unsigned fakeRTSPURLSize = strlen(fakeRTSPURLFmt)
			+ strlen(remoteClientNameOrAddress) + 5/* max port num len */;
...
RTSPRegisterSender* RTSPRegisterSender::createNew(UsageEnvironment& env,
	...
	return new RTSPRegisterSender(env, remoteClientNameOrAddress,
			remoteClientPortNum, rtspURLToRegister, rtspResponseHandler,
			authenticator, requestStreamingViaTCP, proxyURLSuffix,
...
RTSPRegisterSender::RTSPRegisterSender(UsageEnvironment& env,
		...
		RTSPRegisterOrDeregisterSender(env, remoteClientNameOrAddress,
				remoteClientPortNum, authenticator, verbosityLevel,
				applicationName) {
	// Send the "REGISTER" request:
	(void) sendRequest(
			new RequestRecord_REGISTER(++fCSeq, rtspResponseHandler,
...
Boolean RTSPRegisterSender::setRequestFields(RequestRecord* request,
		char*& cmdURL, Boolean& cmdURLWasAllocated, char const*& protocolStr,
	...
	if (strcmp(request->commandName(), "REGISTER") == 0) {
		...
		if (request_REGISTER->proxyURLSuffix() == NULL) {
			...
			unsigned proxyURLSuffixParameterSize = strlen(
					proxyURLSuffixParameterFmt)
		...
		sprintf(transportHeaderStr, transportHeaderFmt,
				...
				request_REGISTER->requestStreamingViaTCP() ?
...
RTSPRegisterSender::RequestRecord_REGISTER::RequestRecord_REGISTER(
		...
		RTSPRegisterOrDeregisterSender::RequestRecord_REGISTER_or_DEREGISTER(
				...
				proxyURLSuffix), fReuseConnection(reuseConnection), fRequestStreamingViaTCP(
...
RTSPDeregisterSender* RTSPDeregisterSender::createNew(UsageEnvironment& env,
		...
		int verbosityLevel, char const* applicationName) {
	return new RTSPDeregisterSender(env, remoteClientNameOrAddress,
			remoteClientPortNum, rtspURLToDeregister, rtspResponseHandler,
			authenticator, proxyURLSuffix, verbosityLevel, applicationName);
}
...
RTSPDeregisterSender::RTSPDeregisterSender(UsageEnvironment& env,
	...
	(void) sendRequest(
			new RequestRecord_DEREGISTER(++fCSeq, rtspResponseHandler,
