...
MPEG4VideoStreamFramer::createNew(UsageEnvironment& env, CommonPlay *cpObj,
		FramedSource* inputSource) {
...
MPEG4VideoStreamFramer::MPEG4VideoStreamFramer(UsageEnvironment& env,
		...
		MPEGVideoStreamFramer(env, cpObj, inputSource), fProfileAndLevelIndication(
				0), fConfigBytes(NULL), fNumConfigBytes(0), fNewConfigBytes(
				NULL), fNumNewConfigBytes(0) {
	fParser =
...
MPEG4VideoStreamParser::MPEG4VideoStreamParser(
		...
		MPEGVideoStreamParser(usingSource, inputSource), fCurrentParseState(
				PARSING_VISUAL_OBJECT_SEQUENCE), vop_time_increment_resolution(
				0), fNumVTIRBits(0), fixed_vop_rate(0), fixed_vop_time_increment(
				0), fSecondsSinceLastTimeCode(0), fTotalTicksSinceLastTimeCode(
				0), fPrevNewTotalTicks(0), fPrevPictureCountDelta(1), fJustSawTimeCode(
				False) {
}
...
void MPEG4VideoStreamParser::analyzeVOLHeader() {
	...
	do {
		...
		fNumVTIRBits = 0;
		for (unsigned test = vop_time_increment_resolution; test > 0; test /=
				2) {
			++fNumVTIRBits;
		}
...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer() {
	...
	setParseState(
			(next4Bytes == GROUP_VOP_START_CODE) ?
					PARSING_GROUP_OF_VIDEO_OBJECT_PLANE :
					PARSING_VIDEO_OBJECT_PLANE);
...
unsigned MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane() {
	...
	unsigned time_code = (next3Bytes[0] << 10) | (next3Bytes[1] << 2)
			| (next3Bytes[2] >> 6);
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane() {
	...
	if (fixed_vop_time_increment > 0) {
		...
		unsigned newTotalTicks = (fSecondsSinceLastTimeCode + modulo_time_base)
				* vop_time_increment_resolution + vop_time_increment;
		if (newTotalTicks == fPrevNewTotalTicks && fPrevNewTotalTicks > 0) {
			...
			if (vop_coding_type != 2/*B*/) {
				int pictureCountDelta = newTotalTicks
						- fTotalTicksSinceLastTimeCode;
