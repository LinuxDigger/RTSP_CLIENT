...
static unsigned const live_tabsel[2][3][16] = { { { 32, 32, 64, 96, 128, 160,
		192, 224, 256, 288, 320, 352, 384, 416, 448, 448 }, { 32, 32, 48, 56,
		64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, 384 }, { 32, 32, 40,
		48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 320 } },
...
static long const live_freqs[] = { 44100, 48000, 32000, 22050, 24000, 16000,
		11025, 12000, 8000, 0 };
...
{ { 0, 4, 8, 12, 16, 20, 24, 30, 36, 44, 52, 62, 74, 90, 110, 134, 162, 196,
		238, 288, 342, 418, 576 }, { 4, 4, 4, 4, 4, 4, 6, 6, 8, 8, 10, 12, 16,
		20, 24, 28, 34, 42, 50, 54, 76, 158 }, { 0, 4 * 3, 8 * 3, 12 * 3, 16
		* 3, 22 * 3, 30 * 3, 40 * 3, 52 * 3, 66 * 3, 84 * 3, 106 * 3, 136 * 3,
		192 * 3 }, { 4, 4, 4, 4, 6, 8, 10, 12, 14, 18, 22, 30, 56 } },

{ { 0, 4, 8, 12, 16, 20, 24, 30, 36, 42, 50, 60, 72, 88, 106, 128, 156, 190,
		230, 276, 330, 384, 576 }, { 4, 4, 4, 4, 4, 4, 6, 6, 6, 8, 10, 12, 16,
		18, 22, 28, 34, 40, 46, 54, 54, 192 }, { 0, 4 * 3, 8 * 3, 12 * 3, 16
		* 3, 22 * 3, 28 * 3, 38 * 3, 50 * 3, 64 * 3, 80 * 3, 100 * 3, 126 * 3,
		192 * 3 }, { 4, 4, 4, 4, 6, 6, 10, 12, 14, 16, 20, 26, 66 } },

{ { 0, 4, 8, 12, 16, 20, 24, 30, 36, 44, 54, 66, 82, 102, 126, 156, 194, 240,
		296, 364, 448, 550, 576 }, { 4, 4, 4, 4, 4, 4, 6, 6, 8, 10, 12, 16, 20,
		24, 30, 38, 46, 56, 68, 84, 102, 26 }, { 0, 4 * 3, 8 * 3, 12 * 3, 16
		* 3, 22 * 3, 30 * 3, 42 * 3, 58 * 3, 78 * 3, 104 * 3, 138 * 3, 180 * 3,
		192 * 3 }, { 4, 4, 4, 4, 6, 8, 12, 16, 20, 26, 34, 42, 12 } },

/* MPEG 2.0 */
{ { 0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 116, 140, 168, 200, 238, 284,
		336, 396, 464, 522, 576 }, { 6, 6, 6, 6, 6, 6, 8, 10, 12, 14, 16, 20,
		24, 28, 32, 38, 46, 52, 60, 68, 58, 54 }, { 0, 4 * 3, 8 * 3, 12 * 3, 18
		* 3, 24 * 3, 32 * 3, 42 * 3, 56 * 3, 74 * 3, 100 * 3, 132 * 3, 174 * 3,
		192 * 3 }, { 4, 4, 4, 6, 6, 8, 10, 14, 18, 26, 32, 42, 18 } },

{ { 0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 114, 136, 162, 194, 232, 278,
		330, 394, 464, 540, 576 }, { 6, 6, 6, 6, 6, 6, 8, 10, 12, 14, 16, 18,
		22, 26, 32, 38, 46, 52, 64, 70, 76, 36 }, { 0, 4 * 3, 8 * 3, 12 * 3, 18
		* 3, 26 * 3, 36 * 3, 48 * 3, 62 * 3, 80 * 3, 104 * 3, 136 * 3, 180 * 3,
		192 * 3 }, { 4, 4, 4, 6, 8, 10, 12, 14, 18, 24, 32, 44, 12 } },

{ { 0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 116, 140, 168, 200, 238, 284,
		336, 396, 464, 522, 576 }, { 6, 6, 6, 6, 6, 6, 8, 10, 12, 14, 16, 20,
		24, 28, 32, 38, 46, 52, 60, 68, 58, 54 }, { 0, 4 * 3, 8 * 3, 12 * 3, 18
		* 3, 26 * 3, 36 * 3, 48 * 3, 62 * 3, 80 * 3, 104 * 3, 134 * 3, 174 * 3,
		192 * 3 }, { 4, 4, 4, 6, 8, 10, 12, 14, 18, 24, 30, 40, 18 } },
...
MP3FrameParams::MP3FrameParams() :
		hdr(0), isMPEG2(false), layer(0), bitrate(0), samplingFreq(0), isStereo(
				false), isFreeFormat(false), frameSize(0), sideInfoSize(0), hasCRC(
				false), bv(frameBytes, 0, sizeof frameBytes) /* by default */, bitrateIndex(
				0), samplingFreqIndex(0), isMPEG2_5(false), padding(false), extension(
				false), mode(0), mode_ext(0), copyright(false), original(false), emphasis(
				0), stereo(0) {
	oldHdr = firstHdr = 0;
...
void MP3FrameParams::setParamsFromHeader() {
	...
	frameSize = ComputeFrameSize(bitrate, samplingFreq, padding, isMPEG2,
			layer);
...
static void getSideInfo2(MP3FrameParams& fr, MP3SideInfo& si, int stereo,
	...
	for (ch = 0; ch < stereo; ch++) {
		...
		if (gr_info.window_switching_flag) {
			...
			r1c = gr_info.region1_count = fr.getBits(3);
			gr_info.region1start = bandInfo[sfreq].longIdx[r0c + 1] >> 1;
			gr_info.region2start = bandInfo[sfreq].longIdx[r0c + 1 + r1c + 1]
					>> 1;
			gr_info.block_type = 0;
			gr_info.mixed_block_flag = 0;
		}
...
static void assignADUBackpointer(MP3FrameParams const& fr, unsigned aduSize,
		MP3SideInfo& sideInfo, unsigned& availableBytesForBackpointer) {
	...
	availableBytesForBackpointer = backpointerSize + fr.frameSize
			- fr.sideInfoSize;
...
unsigned TranscodeMP3ADU(unsigned char const* fromPtr, unsigned fromSize,
	...
	unsigned numAduBits = updateSideInfoSizes(sideInfo, outFr.isMPEG2, fromPtr,
			8 * desiredOutAduSize, part23Length0a, part23Length0aTruncation,
			part23Length0b, part23Length0bTruncation, part23Length1a,
