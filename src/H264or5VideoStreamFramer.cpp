...
Boolean H264or5VideoStreamFramer::isVCL(u_int8_t nal_unit_type) {
	return fHNumber == 264 ?
			(nal_unit_type <= 5 && nal_unit_type > 0) : (nal_unit_type <= 31);
}

////////// H264or5VideoStreamParser implementation //////////

H264or5VideoStreamParser::H264or5VideoStreamParser(int hNumber,
		H264or5VideoStreamFramer* usingSource, FramedSource* inputSource,
		Boolean includeStartCodeInOutput) :
		MPEGVideoStreamParser(usingSource, inputSource), fHNumber(hNumber), fOutputStartCodeSize(
				includeStartCodeInOutput ? 4 : 0), fHaveSeenFirstStartCode(
				False), fHaveSeenFirstByteOfNALUnit(False), fParsedFrameRate(
				0.0), cpb_removal_delay_length_minus1(23), dpb_output_delay_length_minus1(
				23), CpbDpbDelaysPresentFlag(0), pic_struct_present_flag(0), DeltaTfiDivisor(
				2.0) {
}
...
void H264or5VideoStreamParser::removeEmulationBytes(u_int8_t* nalUnitCopy,
		unsigned maxSize, unsigned& nalUnitCopySize) {
	u_int8_t const* nalUnitOrig = fStartOfFrame + fOutputStartCodeSize;
	unsigned const numBytesInNALunit = fTo - nalUnitOrig;
	nalUnitCopySize = removeH264or5EmulationBytes(nalUnitCopy, maxSize,
			nalUnitOrig, numBytesInNALunit);
}
...
void H264or5VideoStreamParser::profile_tier_level(BitVector& bv,
		unsigned max_sub_layers_minus1) {
	bv.skipBits(96);
...
void H264or5VideoStreamParser::analyze_vui_parameters(BitVector& bv,
	...
	if (vcl_hrd_parameters_present_flag)
		analyze_hrd_parameters(bv);
	CpbDpbDelaysPresentFlag = nal_hrd_parameters_present_flag
...
void H264or5VideoStreamParser::analyze_sei_payload(unsigned payloadType,
	...
	if (payloadType == 1/* pic_timing, for both H.264 and H.265 */) {
		...
		if (pic_struct_present_flag) {
			...
			if (DeltaTfiDivisor != prevDeltaTfiDivisor
					&& fParsedFrameRate != 0.0) {
				usingSource()->fFrameRate = fParsedFrameRate = fParsedFrameRate
						* (prevDeltaTfiDivisor / DeltaTfiDivisor);
#ifdef DEBUG
	...
	try {
		...
		if (isVPS(nal_unit_type)) { // Video parameter set
			...
			if (fParsedFrameRate == 0.0) {
				...
				if (time_scale > 0 && num_units_in_tick > 0) {
					usingSource()->fFrameRate = fParsedFrameRate = time_scale
		...
		if (haveSeenEOF() || isEOF(nal_unit_type)) {
			...
			if (isVCL(next_nal_unit_type)) {
				...
				thisNALUnitEndsAccessUnit = (byteAfter_nal_unit_header & 0x80)
						!= 0;
