...
MPEG4VideoStreamDiscreteFramer::createNew(UsageEnvironment& env,
	...
	return new MPEG4VideoStreamDiscreteFramer(env, cpObj, inputSource,
...
MPEG4VideoStreamDiscreteFramer::MPEG4VideoStreamDiscreteFramer(
		...
		Boolean leavePresentationTimesUnmodified) :
		MPEG4VideoStreamFramer(env, cpObj, inputSource,
				False/*don't create a parser*/), fLeavePresentationTimesUnmodified(
				leavePresentationTimesUnmodified), vop_time_increment_resolution(
				0), fNumVTIRBits(0), fLastNonBFrameVop_time_increment(0), fNumBitsSeenSoFar() {
		...
		if (i < frameSize) {
			...
			if (nextCode == 0xB6 /*VOP_START_CODE*/&& i + 5 < frameSize) {
				...
				if (!fLeavePresentationTimesUnmodified && vop_coding_type == 2/*B*/
					...
					unsigned secondsToSubtract = (unsigned) (usIncrement
							/ MILLION);
					unsigned uSecondsToSubtract = ((unsigned) usIncrement)
							% MILLION;
					...
					if ((unsigned) presentationTime.tv_sec
							> secondsToSubtract) {
