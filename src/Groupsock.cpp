...
destRecord* Groupsock::createNewDestRecord(struct in_addr const& addr,
		Port const& port, u_int8_t ttl, unsigned sessionId, destRecord* next) {
...
void Groupsock::changeDestinationParameters(struct in_addr const& newDestAddr,
	...
	destRecord* dest;
	for (dest = fDests; dest != NULL && dest->fSessionId != sessionId; dest =
			dest->fNext) {
	}
	...
	do {
		...
		for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
			if (!write(dests->fGroupEId.groupAddress().s_addr,
					dests->fGroupEId.portNum(), dests->fGroupEId.ttl(), buffer,
					bufferSize)) {
				writeSuccess = False;
				break;
			}
		...
		if (!members().IsEmpty()) {
			numMembers = outputToAllMembersExcept(interfaceNotToFwdBackTo,
					ttl(), buffer, bufferSize, ourIPAddress(env));
			if (numMembers < 0)
				break;
		}
