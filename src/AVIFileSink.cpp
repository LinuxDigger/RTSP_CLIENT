...
public:
	AVIIndexRecord(unsigned chunkId, unsigned flags, unsigned offset,
			unsigned size) :
			fNext(NULL), fChunkId(chunkId), fFlags(flags), fOffset(offset), fSize(
					size) {
	}
...
AVIFileSink::AVIFileSink(UsageEnvironment& env, MediaSession& inputSession,
		...
		Medium(env), fInputSession(inputSession), fIndexRecordsHead(NULL), fIndexRecordsTail(
		NULL), fNumIndexRecords(0), fBufferSize(bufferSize), fPacketLossCompensate(
				packetLossCompensate), fAreCurrentlyBeingPlayed(False), fNumSubsessions(
				0), fNumBytesWritten(0), fHaveCompletedOutputFile(False), fMovieWidth(
				movieWidth), fMovieHeight(movieHeight), fMovieFPS(movieFPS), fcpObj(
				cpObj) {
	...
	while ((subsession = iter.next()) != NULL) {
		...
		AVISubsessionIOState* ioState = new AVISubsessionIOState(*this,
				*subsession);
...
AVIFileSink* AVIFileSink::createNew(UsageEnvironment& env,
		...
		Boolean packetLossCompensate, CommonPlay *cpObj) {
	AVIFileSink* newSink = new AVIFileSink(env, inputSession, outputFileName,
			bufferSize, movieWidth, movieHeight, movieFPS, packetLossCompensate,
			cpObj);
...
void AVIFileSink::onRTCPBye(void* clientData, CommonPlay *cpObj) {
	AVISubsessionIOState* ioState = (AVISubsessionIOState*) clientData;
...
AVISubsessionIOState::AVISubsessionIOState(AVIFileSink& sink,
		MediaSubsession& subsession) :
		fOurSink(sink), fOurSubsession(subsession), fLastPacketRTPSeqNum(), fOurSourceIsActive(), fMaxBytesPerSecond(
				0), fIsVideo(False), fIsAudio(False), fIsByteSwappedAudio(
				False), fAVISubsessionTag(), fAVICodecHandlerType(), fAVISamplingFrequency(), fWAVCodecTag(), fAVIScale(), fAVIRate(), fAVISize(), fNumFrames(
				0), fSTRHFrameCountPosition() {
...
void AVISubsessionIOState::useFrame(SubsessionBuffer& buffer) {
	...
	if (fPrevPresentationTime.tv_usec != 0
			|| fPrevPresentationTime.tv_sec != 0) {
		int uSecondsDiff = (presentationTime.tv_sec
				- fPrevPresentationTime.tv_sec) * 1000000
				+ (presentationTime.tv_usec - fPrevPresentationTime.tv_usec);
		if (uSecondsDiff > 0) {
			unsigned bytesPerSecond = (unsigned) ((frameSize * 1000000.0)
					/ uSecondsDiff);
