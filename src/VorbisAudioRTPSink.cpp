...
char* generateVorbisOrTheoraConfigStr(u_int8_t* identificationHeader,
	...
	unsigned length = identificationHeaderSize + commentHeaderSize
			+ setupHeaderSize;
	...
	*p++ = numHeaders - 1; // "n. of headers"
	if (numHeaders > 1) {
		...
		unsigned length1 =
				identificationHeaderSize > 0 ?
						identificationHeaderSize : commentHeaderSize;
		if (length1 >= 16384) {
	...
	char* base64PackedHeaders = base64Encode((char const*) packedHeaders,
			packedHeadersSize);
