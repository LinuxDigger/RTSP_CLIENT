...
public:
	RTCPMemberDatabase(RTCPInstance& ourRTCPInstance) :
			fOurRTCPInstance(ourRTCPInstance), fNumMembers(1 /*ourself*/), fTable(
...
RTCPInstance::RTCPInstance(UsageEnvironment& env, Groupsock* RTCPgs,
		...
		Medium(env), fcpObj(cpObj), fRTCPInterface(this, RTCPgs), fTotSessionBW(
				...
				0), fLastReceivedSize(0), fLastReceivedSSRC(0), fTypeOfEvent(
		EVENT_UNKNOWN), fTypeOfPacket(PACKET_UNKNOWN_TYPE), fHaveJustSentPacket(
				False), fLastPacketSentSize(0), fByeHandlerTask(NULL), fByeHandlerClientData(
		NULL), fSRHandlerTask(NULL), fSRHandlerClientData(NULL), fRRHandlerTask(
		NULL), fRRHandlerClientData(NULL), fSpecificRRHandlerTable(
...
void RTCPInstance::noteArrivingRR(struct sockaddr_in const& fromAddressAndPort,
	...
	if (fRRHandlerTask != NULL)
		(*fRRHandlerTask)(fRRHandlerClientData, fcpObj);
}
...
Boolean RTCPInstance::lookupByName(UsageEnvironment& env,
		char const* instanceName, RTCPInstance*& resultInstance) {
...
#undef DEBUG
		...
		while (1) {
			...
			case RTCP_PT_SR: {
				...
				if (fSRHandlerTask != NULL)
					(*fSRHandlerTask)(fSRHandlerClientData, fcpObj);
					...
					for (unsigned i = 0; i < rc; ++i) {
						...
						if (senderSSRC == fSink->SSRC()) {
							...
							transmissionStats.noteIncomingRR(reportSenderSSRC,
									fromAddressAndPort, lossStats,
									highestReceived, jitter, timeLastSR,
									timeSinceLastSR);
						} else {
							ADVANCE(4 * 5);
						}
		...
		if (callByeHandler && fByeHandlerTask != NULL/*sanity check*/) {
			...
			(*byeHandler)(fByeHandlerClientData, fcpObj);
		}
	} while (0);
}
