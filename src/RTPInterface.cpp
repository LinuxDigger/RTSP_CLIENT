...
Boolean RTPInterface::handleRead(DP_U8* buffer, unsigned bufferMaxSize,
	...
	if (fNextTCPReadStreamSocketNum < 0) {
		...
		tcpSocketNum = -1;
		readSuccess = fGS->handleRead(buffer, bufferMaxSize, bytesRead,
				fromAddress);
	} else {
		...
		while ((curBytesRead = readSocket(envir(), fNextTCPReadStreamSocketNum,
				&buffer[bytesRead], curBytesToRead, fromAddress)) > 0) {
	...
	if (readSuccess && fAuxReadHandlerFunc != NULL) {
		...
		(*fAuxReadHandlerFunc)(fAuxReadHandlerClientData, buffer, bytesRead); //nothing
	}
	return readSuccess;
}
...
Boolean RTPInterface::sendDataOverTCP(int socketNum, u_int8_t const* data,
	...
	if (sendResult < (int) dataSize) {
		...
		if (numBytesSentSoFar > 0
			...
			sendResult = send(socketNum,
					(char const*) (&data[numBytesSentSoFar]),
...
SocketDescriptor::~SocketDescriptor() {
	...
	if (fSubChannelHashTable != NULL) {
		...
		HashTable::Iterator* iter = HashTable::Iterator::create(
...
#endif
				int result = readSocket(fEnv, fOurSocketNum, &c, 1,
						fromAddress);
