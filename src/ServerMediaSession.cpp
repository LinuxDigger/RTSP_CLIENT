...
ServerMediaSession* ServerMediaSession::createNew(UsageEnvironment& env,
		CommonPlay *cpObj, char const* streamName, char const* info,
		char const* description, Boolean isSSM, char const* miscSDPLines) {
	return new ServerMediaSession(env, cpObj, streamName, info, description,
			isSSM, miscSDPLines);
}
...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env, CommonPlay *cpObj,
		char const* streamName, char const* info, char const* description,
		Boolean isSSM, char const* miscSDPLines) :
		Medium(env), fIsSSM(isSSM), fSubsessionsHead(NULL), fSubsessionsTail(
		NULL), fSubsessionCounter(0), fReferenceCount(0), fDeleteWhenUnreferenced(
				False), fcpObj(cpObj) {
	...
	fDescriptionSDPString = strDup(
			description == NULL ? libNamePlusVersionStr : description);
...
ServerMediaSubsession::ServerMediaSubsession(UsageEnvironment& env,
		...
		Medium(env), fParentSession(NULL), fServerAddressForSDP(0), fPortNumForSDP(
				0), fNext(NULL), fTrackNumber(0), fTrackId(NULL), fcpObj(cpObj) {
}
