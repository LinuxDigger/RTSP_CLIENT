...
unsigned RTSPClient::sendDescribeCommand(responseHandler* responseHandler,
	...
	return sendRequest(
			new RequestRecord(++fCSeq, "DESCRIBE", fcpObj, responseHandler));
}
...
unsigned RTSPClient::sendAnnounceCommand(char const* sdpDescription,
		responseHandler* responseHandler, Authenticator* authenticator) {
	if (fCurrentAuthenticator < authenticator)
		fCurrentAuthenticator = *authenticator;
	return sendRequest(
			new RequestRecord(++fCSeq, "ANNOUNCE", fcpObj, responseHandler,
			NULL,
			NULL, False, 0.0, 0.0, 0.0, sdpDescription));
}

unsigned RTSPClient::sendSetupCommand(MediaSubsession& subsession,
		responseHandler* responseHandler, Boolean streamOutgoing,
	...
	if (forceMulticastOnUnspecified)
		booleanFlags |= 0x4;
	return sendRequest(
			new RequestRecord(++fCSeq, "SETUP", fcpObj, responseHandler, NULL,
					&subsession, booleanFlags));
}
...
unsigned RTSPClient::sendPlayCommand(MediaSession& session,
	...
	return sendRequest(
			new RequestRecord(++fCSeq, "PLAY", fcpObj, responseHandler,
					&session,
					NULL, 0, start, end, scale));
}
...
unsigned RTSPClient::sendPlayCommand(MediaSubsession& subsession,
	...
	return sendRequest(
			new RequestRecord(++fCSeq, "PLAY", fcpObj, responseHandler, NULL,
					&subsession, 0, start, end, scale));
}

unsigned RTSPClient::sendPlayCommand(MediaSession& session,
		responseHandler* responseHandler, char const* absStartTime,
		char const* absEndTime, float scale, Authenticator* authenticator) {
	if (fCurrentAuthenticator < authenticator)
		fCurrentAuthenticator = *authenticator;
	sendDummyUDPPackets(session); // hack to improve NAT traversal
	return sendRequest(
			new RequestRecord(++fCSeq, responseHandler, fcpObj, absStartTime,
...
unsigned RTSPClient::sendPauseCommand(MediaSession& session,
	...
	if (fCurrentAuthenticator < authenticator)
		fCurrentAuthenticator = *authenticator;
	return sendRequest(
			new RequestRecord(++fCSeq, "PAUSE", fcpObj, responseHandler,
					&session));
}
...
unsigned RTSPClient::sendPauseCommand(MediaSubsession& subsession,
	...
	if (fCurrentAuthenticator < authenticator)
		fCurrentAuthenticator = *authenticator;
	return sendRequest(
			new RequestRecord(++fCSeq, "PAUSE", fcpObj, responseHandler, NULL,
					&subsession));
}
...
unsigned RTSPClient::sendRecordCommand(MediaSession& session,
	...
	if (fCurrentAuthenticator < authenticator)
		fCurrentAuthenticator = *authenticator;
	return sendRequest(
			new RequestRecord(++fCSeq, "RECORD", fcpObj, responseHandler,
					&session));
}
...
unsigned RTSPClient::sendRecordCommand(MediaSubsession& subsession,
	...
	if (fCurrentAuthenticator < authenticator)
		fCurrentAuthenticator = *authenticator;
	return sendRequest(
			new RequestRecord(++fCSeq, "RECORD", fcpObj, responseHandler, NULL,
					&subsession));
}
...
unsigned RTSPClient::sendTeardownCommand(MediaSession& session,
	...
	if (fCurrentAuthenticator < authenticator)
		fCurrentAuthenticator = *authenticator;
	return sendRequest(
			new RequestRecord(++fCSeq, "TEARDOWN", fcpObj, responseHandler,
					&session));
}
...
unsigned RTSPClient::sendSetParameterCommand(MediaSession& session,
	...
	unsigned result = sendRequest(
			new RequestRecord(++fCSeq, "SET_PARAMETER", fcpObj, responseHandler,
					&session, NULL, False, 0.0, 0.0, 0.0, paramString));
	delete[] paramString;
	return result;
}
...
unsigned RTSPClient::sendGetParameterCommand(MediaSession& session,
	...
	unsigned result = sendRequest(
			new RequestRecord(++fCSeq, "GET_PARAMETER", fcpObj, responseHandler,
					&session, NULL, False, 0.0, 0.0, 0.0, paramString));
	delete[] paramString;
	cout << "seg 222"<<endl;
	return result;
...
Boolean RTSPClient::changeResponseHandler(unsigned cseq,
	...
	if ((request = fRequestsAwaitingConnection.findByCSeq(cseq)) != NULL
			|| (request = fRequestsAwaitingHTTPTunneling.findByCSeq(cseq))
					!= NULL
...
Boolean RTSPClient::lookupByName(UsageEnvironment& env,
		char const* instanceName, RTSPClient*& resultClient) {
...
Boolean RTSPClient::parseRTSPURL(UsageEnvironment& env, char const* url,
		char*& username, char*& password, NetAddress& address,
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
		...
		NULL), fUserAgentHeaderStrLen(0), fInputSocketNum(-1), fOutputSocketNum(
				-1), fBaseURL(NULL), fTCPStreamIdCount(0), fLastSessionId(NULL), fSessionTimeoutParameter(
				0), fSessionCookieCounter(0), fHTTPTunnelingConnectionIsPending(
				False) {
	setBaseURL(rtspURL);
	...
	if (socketNumToServer >= 0) {
		...
		envir().taskScheduler(fcpObj->_fClientID / 100)->setBackgroundHandling(
				fInputSocketNum,
...
int RTSPClient::grabSocket() {
	...
	RTPInterface::clearServerRequestAlternativeByteHandler(
			fcpObj->_fClientID / 100, envir(), fInputSocketNum); // in case we were receiving RTP-over-TCP
	fInputSocketNum = -1;
	...
	do {
		...
		if (!fRequestsAwaitingConnection.isEmpty()) {
			...
			Logger::GetInstance().Info(
					...
					fInputSocketNum);
			envir().fScheduler[fcpObj->_fClientID / 100]->_mSockfdCpSet[fInputSocketNum] =
					fcpObj;
		}
		...
		unsigned cmdSize = strlen(cmdFmt) + strlen(request->commandName())
				+ strlen(cmdURL) + strlen(protocolStr) + 20 /* max int len */
...
Boolean RTSPClient::setRequestFields(RequestRecord* request, char*& cmdURL,
		Boolean& cmdURLWasAllocated, char const*& protocolStr,
	...
	if (strcmp(request->commandName(), "DESCRIBE") == 0) {
	...
	} else if (strcmp(request->commandName(), "OPTIONS") == 0) {  // ''option
		...
		Boolean forceMulticastOnUnspecified = (request->booleanFlags() & 0x4)
				!= 0;
		...
		if (streamUsingTCP) { // streaming over the RTSP connection
			...
			Boolean requestMulticastStreaming = IsMulticastAddress(
					connectionAddress)
		...
		unsigned transportSize = strlen(transportFmt) + strlen(transportTypeStr)
				+ strlen(modeStr) + strlen(portTypeStr)
				+ 2 * 5 /* max port len */;
...
void RTSPClient::resetTCPSockets() {
	if (fInputSocketNum >= 0) {
		RTPInterface::clearServerRequestAlternativeByteHandler(
				fcpObj->_fClientID / 100, envir(), fInputSocketNum); // in case we were receiving RTP-over-TCP
		envir().taskScheduler(fcpObj->_fClientID / 100)->disableBackgroundHandling(
				fInputSocketNum);
...
int RTSPClient::openConnection() {
	do {
		...
		portNumBits destPortNum =
				fTunnelOverHTTPPortNum == 0 ?
						urlPortNum : fTunnelOverHTTPPortNum;
	...
	if (auth.realm() != NULL && auth.username() != NULL
		...
		if (auth.nonce() != NULL) { // Digest authentication
			...
			unsigned authBufSize = strlen(authFmt) + strlen(auth.username())
					+ strlen(auth.realm()) + strlen(auth.nonce()) + strlen(url)
					+ strlen(response);
			...
			unsigned usernamePasswordLength = strlen(auth.username()) + 1
...
void RTSPClient::handleIncomingRequest() {
	...
	if (!parseRTSPRequestString(fResponseBuffer, fResponseBytesAlreadySeen,
			cmdName, sizeof cmdName, urlPreSuffix, sizeof urlPreSuffix,
			urlSuffix, sizeof urlSuffix, cseq, sizeof cseq, sessionId,
...
Boolean RTSPClient::handleSETUPResponse(MediaSubsession& subsession,
		char const* sessionParamsStr, char const* transportParamsStr,
	...
	do {
		...
		if (streamUsingTCP) {
			...
			RTPInterface::setServerRequestAlternativeByteHandler(
					fcpObj->_fClientID / 100, envir(), fInputSocketNum,
...
Boolean RTSPClient::handlePLAYResponse(MediaSession& session,
		MediaSubsession& subsession, char const* scaleParamsStr,
		char const* speedParamsStr, char const* rangeParamsStr,
		...
		if (&session != NULL) {
			...
			if (rangeParamsStr != NULL
					&& !parseRangeParam(rangeParamsStr, session.playStartTime(),
							session.playEndTime(), session._absStartTime(),
...
Boolean RTSPClient::handleGET_PARAMETERResponse(char const* parameterName,
	...
	do {
		...
		if (parameterName != NULL && parameterName[0] != '\0') {
			...
			if (_strncasecmp(resultValueString, parameterName, parameterNameLen)
					== 0) {
		...
		*resultValueStringEnd = saved;
		...
		while (resultLen > 0
				&& (resultValueString[resultLen - 1] == '\r'
...
Boolean RTSPClient::handleAuthenticationFailure(char const* paramsStr) {
	...
	if (sscanf(paramsStr,
			"Digest realm=\"%[^\"]\", nonce=\"%[^\"]\", stale=%[a-zA-Z]", realm,
			nonce, stale) == 3) {
...
void RTSPClient::constructSubsessionURL(MediaSubsession const& subsession,
		char const*& prefix, char const*& separator, char const*& suffix) {
	...
	if (isAbsoluteURL(suffix)) {
		...
		separator =
				(prefixLen == 0 || prefix[prefixLen - 1] == '/'
						|| suffix[0] == '/') ? "" : "/";
	}
}
...
Boolean RTSPClient::setupHTTPTunneling1() {
	...
	return sendRequest(
			new RequestRecord(1, "GET", fcpObj, responseHandlerForHTTP_GET))
			!= 0;
}
...
void RTSPClient::connectionHandler1() {
	...
	do {
		...
		while ((request = tmpRequestQueue.dequeue()) != NULL) { // dequeue -> request =NULL or next
			sendRequest(request);   ///sending
		}
		return;
	} while (0);
	...
	do {
		...
		do {
				...
				if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
					...
					while ((request = fRequestsAwaitingResponse.dequeue())
						...
						if (request->cseq() < cseq) { // assumes that the CSeq counter will never wrap around
							...
							if (fVerbosityLevel >= 1
									&& strcmp(request->commandName(), "POST")
											!= 0) {
								envir()
										<< "WARNING: The server did not respond to our \""
										<< request->commandName()
										<< "\" request (CSeq: "
										<< request->cseq()
					...
					setBaseURL(headerParamsStr);
						...
						scaleParamsStr)) {
				} else if (checkForHeader(lineStart, "Speed:", 6,
						speedParamsStr)) {
					...
					if (wwwAuthenticateParamsStr == NULL
							|| _strncasecmp(headerParamsStr, "Digest", 6)
									== 0) {
					...
					if (oldBaseURLTail != NULL) {
						unsigned newBaseURLLen = (oldBaseURLTail - fBaseURL) + 8/* for "/stream=" */
			...
			if (contentLength > numBodyBytes) {
				...
				unsigned remainingBufferSize = responseBufferSize
						- fResponseBytesAlreadySeen;
				...
				if (fVerbosityLevel >= 1) {
					envir() << "Have received " << fResponseBytesAlreadySeen
							<< " total bytes of a "
							<< (foundRequest != NULL ?
									foundRequest->commandName() : "(unknown)")
					...
					if (strcmp(foundRequest->commandName(), "SETUP") == 0) {
						...
						if (!handlePLAYResponse(*foundRequest->session(),
								*foundRequest->subsession(), scaleParamsStr,
								speedParamsStr, rangeParamsStr,
								rtpInfoParamsStr))
							break;
					} else if (strcmp(foundRequest->commandName(), "TEARDOWN")
							== 0) {
		...
		if (foundRequest != NULL && foundRequest->handler() != NULL) {
			...
			if (responseSuccess) {
				if (responseCode == 200) {
					...
					resultString =
							numBodyBytes > 0 ?
									strDup(bodyStart) : strDup(publicParamsStr);
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, char const* commandName,
		...
		double end, float scale, char const* contentStr) :
		fNext(NULL), fCSeq(cseq), fCommandName(commandName), fSession(session), fSubsession(
				subsession), fBooleanFlags(booleanFlags), fStart(start), fEnd(
				end), fAbsStartTime(NULL), fAbsEndTime(NULL), fScale(scale), fContentStr(
				strDup(contentStr)), fHandler(handler), fcpObj(cpObj) {
}

RTSPClient::RequestRecord::RequestRecord(unsigned cseq,
		responseHandler* handler, CommonPlay *cpObj, char const* absStartTime,
		char const* absEndTime, float scale, MediaSession* session,
		MediaSubsession* subsession) :
		fNext(NULL), fCSeq(cseq), fCommandName("PLAY"), fSession(session), fSubsession(
				subsession), fBooleanFlags(0), fStart(0.0f), fEnd(-1.0f), fAbsStartTime(
				strDup(absStartTime)), fAbsEndTime(strDup(absEndTime)), fScale(
				scale), fContentStr(NULL), fHandler(handler), fcpObj(cpObj) {
}
