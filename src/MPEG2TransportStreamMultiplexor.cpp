...
MPEG2TransportStreamMultiplexor::MPEG2TransportStreamMultiplexor(
		...
		FramedSource(env, cpObj), fHaveVideoStreams(True/*by default*/), fOutgoingPacketCounter(
				...
				0), fIsFirstAdaptationField(True) {
	...
	if (stream_id == 0xBE) { // padding_stream; ignore
		...
		if (streamType == 0) {
			...
			if ((stream_id & 0xF0) == 0xE0) { // video
				streamType =
						mpegVersion == 1 ? 1 : mpegVersion == 2 ? 2 :
						mpegVersion == 4 ? 0x10 :
		...
		*header++ = adaptation_field_control | (continuity_counter & 0x0F);
		...
		if (adaptation_field_control == 0x30) {
			...
			u_int8_t adaptation_field_length =
					(numHeaderBytes == 5) ?
							0 : 1 + numPCRBytes + numPaddingBytes;
			*header++ = adaptation_field_length;
			if (numHeaderBytes > 5) {
				...
				*header++ = flags;
				if (willAddPCR) {
					u_int32_t pcrHigh32Bits = (fPCR.highBit << 31)
							| (fPCR.remainingBits >> 1);
...
void MPEG2TransportStreamMultiplexor::setProgramStreamMap(unsigned frameSize) {
	...
	u_int16_t program_stream_map_length = (fInputBuffer[4] << 8)
			| fInputBuffer[5];
	...
	u_int16_t program_stream_info_length = (fInputBuffer[8] << 8)
			| fInputBuffer[9];
	unsigned offset = 10 + program_stream_info_length; // skip over 'descriptors'

	u_int16_t elementary_stream_map_length = (fInputBuffer[offset] << 8)
			| fInputBuffer[offset + 1];
	offset += 2;
...
static u_int32_t const CRC32[256] = { 0x00000000, 0x04c11db7, 0x09823b6e,
