...
DP_U32 CycleQueue::DSP_PutData(const void *buff, DP_U32 size) {
	...
	memcpy(m_cycleQueue->_buffer, (char*)buff + len, size - len);
...
DP_U32 CycleQueue::DSP_GetDate(void *buff, DP_U32 len) {
	DP_U32 ret = 0;
	pthread_mutex_lock(m_cycleQueue->_mutex);
	DP_U32 size = 0;
	...
	memcpy(buff,
			(char*)m_cycleQueue->_buffer
					+ (m_cycleQueue->_out & (m_cycleQueue->_size - 1)), size);
	/* then get the rest (if any) from the beginning of the buffer */
	memcpy((char*)buff + size, m_cycleQueue->_buffer, len - size);
