			...
			if (clientRTCPPort.num() == 0) {
				...
				for (portNumBits serverPortNum = fInitialPortNum;;
					...
					if (fMultiplexRTCPWithRTP) {
						...
						rtcpGroupsock = createGroupsock(dummyAddr,
								serverRTCPPort);
...
void OnDemandServerMediaSubsession::sendRTCPAppPacket(u_int8_t subtype,
		char const* name, u_int8_t* appDependentData,
...
StreamState::StreamState(OnDemandServerMediaSubsession& master,
		...
		fMaster(master), fAreCurrentlyPlaying(False), fReferenceCount(1), fServerRTPPort(
