...
#include "H263plusVideoRTPSource.h" // for the special header
#include "MPEG4GenericRTPSource.h" //for "samplingFrequencyFromAudioSpecificConfig()"
...
QuickTimeFileSink::QuickTimeFileSink(UsageEnvironment& env,
		...
		unsigned bufferSize, unsigned short movieWidth,
		unsigned short movieHeight, unsigned movieFPS,
		Boolean packetLossCompensate, Boolean syncStreams,
		Boolean generateHintTracks, Boolean generateMP4Format) :
		Medium(env), fInputSession(inputSession), fBufferSize(bufferSize), fPacketLossCompensate(
				packetLossCompensate), fSyncStreams(syncStreams), fGenerateMP4Format(
				generateMP4Format), fAreCurrentlyBeingPlayed(False), fLargestRTPtimestampFrequency(
				0), fNumSubsessions(0), fNumSyncedSubsessions(0), fHaveCompletedOutputFile(
				False), fMovieWidth(movieWidth), fMovieHeight(movieHeight), fMovieFPS(
				movieFPS), fMaxTrackDurationM(0) {
	...
	while ((subsession = iter.next()) != NULL) {
		...
		if (generateHintTracks) {
			...
			SubsessionIOState* hintTrack = new SubsessionIOState(*this,
					*subsession);
...
QuickTimeFileSink*
QuickTimeFileSink::createNew(UsageEnvironment& env, MediaSession& inputSession,
		...
		Boolean generateHintTracks, Boolean generateMP4Format) {
	QuickTimeFileSink* newSink = new QuickTimeFileSink(env, inputSession,
			outputFileName, bufferSize, movieWidth, movieHeight, movieFPS,
...
void QuickTimeFileSink::noteRecordedFrame(MediaSubsession& /*inputSubsession*/,
...
void QuickTimeFileSink::afterGettingFrame(void* clientData,
		unsigned packetDataSize, unsigned numTruncatedBytes,
...
SubsessionIOState::SubsessionIOState(QuickTimeFileSink& sink,
		MediaSubsession& subsession) :
		fHintTrackForUs(NULL), fTrackHintedByUs(NULL), fOurSink(sink), fOurSubsession(
				subsession), fLastPacketRTPSeqNum(0), fHaveBeenSynced(False), fQTTotNumSamples(
				0), fHeadChunk(NULL), fTailChunk(NULL), fNumChunks(0), fHeadSyncFrame(
				NULL), fTailSyncFrame(NULL) {
	fTrackID = ++fCurrentTrackNumber;

	fBuffer = new SubsessionBuffer(fOurSink.fBufferSize);
	fPrevBuffer =
			sink.fPacketLossCompensate ?
...
void SubsessionIOState::useFrame(SubsessionBuffer& buffer) {
	...
	if (!fOurSink.fSyncStreams
			|| fQTcomponentSubtype != fourChar('v', 'i', 'd', 'e')) {
		unsigned const frameDuration = fQTTimeUnitsPerSample
				* fQTSamplesPerFrame;
		...
		if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
			...
			if (duration < 0.0)
				duration = 0.0;
			unsigned frameDuration = (unsigned) ((2 * duration * fQTTimeScale
					+ 1) / 2); // round
...
void SubsessionIOState::useFrameForHinting(unsigned frameSize,
		struct timeval presentationTime, unsigned startSampleNumber) {
	...
	if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
		...
		unsigned hintSampleDuration = (unsigned) ((2 * duration * fQTTimeScale
				+ 1) / 2); // round
		if (hackm4a) {
		...
		for (unsigned i = 0; i < numPTEntries; ++i) {
			...
			if (haveSpecialHeaders) { // special case
				...
				dataFrameSize = fPrevFrameState.packetSizes[i]
						- immediateDataLen;
	...
	if (hack263) {
		...
		unsigned const sizeLength = fOurSubsession.attrVal_unsigned(
				"sizelength");
		unsigned const indexLength = fOurSubsession.attrVal_unsigned(
				"indexlength");
		...
		fPrevFrameState.specialHeaderBytes[2] = ((frameSize << indexLength)
				& 0xFF00) >> 8;
...
unsigned SubsessionIOState::useFrame1(unsigned sourceDataSize,
		struct timeval presentationTime, unsigned frameDuration,
	...
	if (fTailChunk == NULL) {
		newTailChunk = fHeadChunk = new ChunkDescriptor(destFileOffset,
				sourceDataSize, frameSize, frameDuration, presentationTime);
	} else {
...
ChunkDescriptor::ChunkDescriptor(int64_t offsetInFile, unsigned size,
		...
		struct timeval presentationTime) :
		fNextChunk(NULL), fOffsetInFile(offsetInFile), fNumFrames(
				size / frameSize), fFrameSize(frameSize), fFrameDuration(
				frameDuration), fPresentationTime(presentationTime) {
}

ChunkDescriptor* ChunkDescriptor::extendChunk(int64_t newOffsetInFile,
		unsigned newSize, unsigned newFrameSize, unsigned newFrameDuration,
		struct timeval newPresentationTime) {
	...
	ChunkDescriptor* newDescriptor = new ChunkDescriptor(newOffsetInFile,
			newSize, newFrameSize, newFrameDuration, newPresentationTime);
...
if (totalDurationOfEdits > fCurrentIOState->fQTDurationM) {
	...
	double scaleFactor = fCurrentIOState->fQTTimeScale
			/ (double) movieTimeScale();
	fCurrentIOState->fQTDurationT = (unsigned) (totalDurationOfEdits
			* scaleFactor);
}
addAtomEnd
;
...
if (fCurrentIOState->fHeadSyncFrame != NULL) {
	...
	for (ChunkDescriptor* chunk = fCurrentIOState->fHeadChunk; chunk != NULL;
			chunk = chunk->fNextChunk)
...
if (!haveSingleEntryTable) {
	...
	ChunkDescriptor* chunk = fCurrentIOState->fHeadChunk;
	while (chunk != NULL) {
		unsigned numSamples = chunk->fNumFrames
				* (fCurrentIOState->fQTSamplesPerFrame);
		unsigned sampleSize = chunk->fFrameSize
				/ (fCurrentIOState->fQTSamplesPerFrame);
