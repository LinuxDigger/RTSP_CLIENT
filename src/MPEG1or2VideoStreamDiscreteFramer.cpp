...
MPEG1or2VideoStreamDiscreteFramer::createNew(UsageEnvironment& env,
		CommonPlay *cpObj, FramedSource* inputSource, Boolean iFramesOnly,
		double vshPeriod, Boolean leavePresentationTimesUnmodified) {
	// Need to add source type checking here???  #####
	return new MPEG1or2VideoStreamDiscreteFramer(env, cpObj, inputSource,
...
MPEG1or2VideoStreamDiscreteFramer::MPEG1or2VideoStreamDiscreteFramer(
		...
		MPEG1or2VideoStreamFramer(env, cpObj, inputSource, iFramesOnly,
				vshPeriod, False/*don't create a parser*/), fLeavePresentationTimesUnmodified(
				leavePresentationTimesUnmodified), fLastNonBFrameTemporal_reference(
				0), fSavedVSHSize(0), fSavedVSHTimestamp(0.0), fIFramesOnly(
				iFramesOnly), fVSHPeriod(vshPeriod) {
...
static double const frameRateFromCode[] = { 0.0,          // forbidden
		24000 / 1001.0, // approx 23.976
		24.0, 25.0, 30000 / 1001.0, // approx 29.97
		30.0, 50.0, 60000 / 1001.0, // approx 59.94
		60.0, 0.0,          // reserved
		0.0,          // reserved
		0.0,          // reserved
		0.0,          // reserved
	...
	if (frameSize >= 4 && fTo[0] == 0 && fTo[1] == 0 && fTo[2] == 1) {
		...
		if (nextCode == 0x00 /*PICTURE_START_CODE*/&& i + 2 < frameSize) {
			...
			if (!fLeavePresentationTimesUnmodified && picture_coding_type == 3/*B*/
					&& (fLastNonBFramePresentationTime.tv_usec > 0
							|| fLastNonBFramePresentationTime.tv_sec > 0)) {
				int trIncrement = fLastNonBFrameTemporal_reference
						- temporal_reference;
				if (trIncrement < 0)
