...
MPEG1or2VideoStreamFramer::MPEG1or2VideoStreamFramer(UsageEnvironment& env,
		...
		double vshPeriod, Boolean createParser) :
		MPEGVideoStreamFramer(env, cpObj, inputSource) {
	fParser =
			createParser ?
...
MPEG1or2VideoStreamFramer::createNew(UsageEnvironment& env, CommonPlay *cpObj,
		FramedSource* inputSource, Boolean iFramesOnly, double vshPeriod) {
	// Need to add source type checking here???  #####
	return new MPEG1or2VideoStreamFramer(env, cpObj, inputSource, iFramesOnly,
			vshPeriod);
}
...
MPEG1or2VideoStreamParser::MPEG1or2VideoStreamParser(
		...
		MPEGVideoStreamParser(usingSource, inputSource), fCurrentParseState(
				PARSING_VIDEO_SEQUENCE_HEADER), fVSHPeriod(vshPeriod), fIFramesOnly(
				iFramesOnly) {
	reset();
}
...
unsigned MPEG1or2VideoStreamParser::parseGOPHeader(Boolean haveSeenStartCode) {
	...
	usingSource()->setTimeCode(time_code_hours, time_code_minutes,
			time_code_seconds, time_code_pictures, fPicturesSinceLastGOP);
