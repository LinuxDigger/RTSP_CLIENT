/*
 * MEPG2TransportStreamFramer.cpp
 *
...
public:
	PIDStatus(double _firstClock, double _firstRealTime) :
			firstClock(_firstClock), lastClock(_firstClock), firstRealTime(
					_firstRealTime), lastRealTime(_firstRealTime), lastPacketNum(
					0) {
	}
...
MPEG2TransportStreamFramer* MPEG2TransportStreamFramer::createNew(
	...
	return new MPEG2TransportStreamFramer(env, cpObj, inputSource);
}

MPEG2TransportStreamFramer::MPEG2TransportStreamFramer(UsageEnvironment& env,
		CommonPlay *cpObj, FramedSource* inputSource) :
		FramedFilter(env, cpObj, inputSource), fTSPacketCount(0), fTSPacketDurationEstimate(
				0.0), fTSPCRCount(0), fLimitNumTSPacketsToStream(False), fNumTSPacketsToStream(
				0), fLimitTSPacketsToStreamByPCR(False), fPCRLimit(0.0) {
...
void MPEG2TransportStreamFramer::afterGettingFrame1(unsigned frameSize,
	...
	fDurationInMicroseconds = numTSPackets
			* (unsigned) (fTSPacketDurationEstimate * 1000000);
...
Boolean MPEG2TransportStreamFramer::updateTSPacketDurationEstimate(
	...
	++fTSPCRCount;
	u_int32_t pcrBaseHigh = (pkt[6] << 24) | (pkt[7] << 16) | (pkt[8] << 8)
			| pkt[9];
	...
	if (pidStatus == NULL) {
		...
		int64_t packetsSinceLast = (int64_t) (fTSPacketCount
				- pidStatus->lastPacketNum);
		// it's "int64_t" because some compilers can't convert "u_int64_t" -> "double"
		double durationPerPacket = (clock - pidStatus->lastClock)
				/ packetsSinceLast;
