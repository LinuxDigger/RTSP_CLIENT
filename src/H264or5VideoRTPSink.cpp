...
public:
	H264or5Fragmenter(int hNumber, UsageEnvironment& env, CommonPlay *cpObj,
...
H264or5VideoRTPSink::H264or5VideoRTPSink(int hNumber, UsageEnvironment& env,
		...
		VideoRTPSink(env, cpObj, RTPgs, rtpPayloadFormat, 90000,
				hNumber == 264 ? "H264" : "H265"), fHNumber(hNumber), fOurFragmenter(
...
Boolean H264or5VideoRTPSink::continuePlaying() {
	...
	if (fOurFragmenter == NULL) {
		fOurFragmenter = new H264or5Fragmenter(fHNumber, envir(), fcpObj,
...
H264or5Fragmenter::H264or5Fragmenter(int hNumber, UsageEnvironment& env,
		CommonPlay *cpObj, FramedSource* inputSource, unsigned inputBufferMax,
		unsigned maxOutputPacketSize) :
