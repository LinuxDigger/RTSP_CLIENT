...
MP3HuffmanEncodingInfo::MP3HuffmanEncodingInfo(Boolean includeDecodedValues) :
		numSamples(0), reg1Start(0), reg2Start(0), bigvalStart(0) {
...
void updateSideInfoForHuffman(MP3SideInfo& sideInfo, Boolean isMPEG2,
	...
	if (isMPEG2) {
		...
		unsigned granule1Offset = origTotABsize
...
static unsigned char const stab[3][6][4] = { { { 6, 5, 5, 5 }, { 6, 5, 7, 3 }, {
		...
		18, 0, 0 }, { 6, 15, 12, 0 }, { 6, 12, 9, 6 }, { 6, 18, 9, 0 } } };
...
static int rsf_huffman_decoder(BitVector& bv, struct huffcodetab const* h, // ptr to huffman code record
