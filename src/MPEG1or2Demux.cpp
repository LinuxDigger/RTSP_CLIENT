...
enum MPEGParseState {
	PARSING_PACK_HEADER, PARSING_SYSTEM_HEADER, PARSING_PES_PACKET
};
...
void MPEG1or2Demux::registerReadInterest(u_int8_t streamIdTag,
	...
	if (out.isCurrentlyAwaitingData) {
		envir()
				<< "MPEG1or2Demux::registerReadInterest(): attempt to read stream more than once!\n";
		envir().internalError();
	}
...
void MPEG1or2Demux::getNextFrame(u_int8_t streamIdTag, unsigned char* to,
	...
	registerReadInterest(streamIdTag, to, maxSize, afterGettingFunc,
			afterGettingClientData, onCloseFunc, onCloseClientData);
...
MPEGProgramStreamParser::MPEGProgramStreamParser(MPEG1or2Demux* usingDemux,
		...
		StreamParser(inputSource, MPEG1or2Demux::handleClosure, usingDemux,
				&MPEG1or2Demux::continueReadProcessing, usingDemux), fUsingDemux(
				usingDemux), fCurrentParseState(PARSING_PACK_HEADER) {
}
...
void MPEGProgramStreamParser::parsePackHeader() {
	...
	if ((nextByte & 0xF0) == 0x20) { // MPEG-1
		...
		fUsingDemux->envir()
				<< "StreamParser::parsePack() saw strange byte following pack_start_code\n";
	}
