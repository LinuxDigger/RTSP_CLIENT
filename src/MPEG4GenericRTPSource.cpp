...
MPEG4GenericRTPSource::createNew(UsageEnvironment& env, CommonPlay *cpObj,
	...
	return new MPEG4GenericRTPSource(env, cpObj, RTPgs, rtpPayloadFormat,
			rtpTimestampFrequency, mediumName, mode, sizeLength, indexLength,
			indexDeltaLength);
}
...
MPEG4GenericRTPSource::MPEG4GenericRTPSource(UsageEnvironment& env,
		...
		MultiFramedRTPSource(env, cpObj, RTPgs, rtpPayloadFormat,
				rtpTimestampFrequency, new MPEG4GenericBufferedPacketFactory), fSizeLength(
				sizeLength), fIndexLength(indexLength), fIndexDeltaLength(
				indexDeltaLength), fNumAUHeaders(0), fNextAUHeader(0), fAUHeaders(
				NULL) {
	unsigned mimeTypeLength = strlen(mediumName) + 14 /* strlen("/MPEG4-GENERIC") */
			+ 1;
...
static unsigned const samplingFrequencyFromIndex[16] = { 96000, 88200, 64000,
		48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000, 7350, 0,
		0, 0 };
