...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env, CommonPlay *cpObj,
		...
		RTPSink(env, rtpGS, rtpPayloadType, rtpTimestampFrequency,
				rtpPayloadFormatName, numChannels, cpObj), fOutBuf(
		NULL), fCurFragmentationOffset(0), fPreviousFrameEndedFragmentation(
...
void MultiFramedRTPSink::afterGettingFrame1(unsigned frameSize,
	...
	if (fNumFramesUsedSoFar > 0) {
		if ((fPreviousFrameEndedFragmentation
				...
				|| !frameCanAppearAfterPacketStart(fOutBuf->curPtr(),
						frameSize)) {
