...
RTPSink::RTPSink(UsageEnvironment& env, Groupsock* rtpGS,
		...
		MediaSink(env,cpObj), fRTPInterface(cpObj,this, rtpGS), fRTPPayloadType(
				rtpPayloadType), fPacketCount(0), fOctetCount(0), fTotalOctetCount(
				0), fTimestampFrequency(rtpTimestampFrequency), fNextTimestampHasBeenPreset(
				False), fEnableRTCPReports(True), fNumChannels(numChannels), fEstimatedBitrate(
				0) {
	fRTPPayloadFormatName = strDup(
			rtpPayloadFormatName == NULL ? "???" : rtpPayloadFormatName);
...
u_int32_t RTPSink::convertToRTPTimestamp(struct timeval tv) {
	...
	timestampIncrement += (u_int32_t) (fTimestampFrequency
			* (tv.tv_usec / 1000000.0) + 0.5); // note: rounding
...
void RTPSink::getTotalBitrate(unsigned& outNumBytes, double& outElapsedTime) {
	...
	outElapsedTime =
			(double) (timeNow.tv_sec - fTotalOctetCountStartTime.tv_sec)
					+ (timeNow.tv_usec - fTotalOctetCountStartTime.tv_usec)
							/ 1000000.0;
...
char* RTPSink::rtpmapLine() const {
	if (rtpPayloadType() >= 96) { // the payload format type is dynamic
		...
		+ strlen(rtpPayloadFormatName()) + 20 /* max int len */
...
RTPTransmissionStats::RTPTransmissionStats(RTPSink& rtpSink, u_int32_t SSRC) :
		fOurRTPSink(rtpSink), fSSRC(SSRC), fLastPacketNumReceived(0), fPacketLossRatio(
				0), fTotNumPacketsLost(0), fJitter(0), fLastSRTime(0), fDiffSR_RRTime(
				0), fAtLeastTwoRRsHaveBeenReceived(False), fOldLastPacketNumReceived(
				0), fOldTotNumPacketsLost(0), fFirstPacket(True), fFirstPacketNumReported(
				0), fTotalOctetCount_hi(0), fTotalOctetCount_lo(0), fTotalPacketCount_hi(
				0), fTotalPacketCount_lo(0) {
...
unsigned RTPTransmissionStats::roundTripDelay() const {
	...
	unsigned lastReceivedTimeNTP = (unsigned) ((lastReceivedTimeNTP_high << 16)
			+ fractionalPart + 0.5);
