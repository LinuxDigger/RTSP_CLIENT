...
#ifndef INCLUDE_RTSPCLIENT_H_
#define INCLUDE_RTSPCLIENT_H_
...
public:
	...
	unsigned sendSetupCommand(MediaSubsession& subsession,
			responseHandler* responseHandler, Boolean streamOutgoing = False,
	...
	unsigned sendPlayCommand(MediaSession& session,
			responseHandler* responseHandler, double start = 0.0f, double end =
					-1.0f, float scale = 1.0f, Authenticator* authenticator =
			NULL);
	...
	unsigned sendSetParameterCommand(MediaSession& session,
			...
			char const* parameterValue, Authenticator* authenticator = NULL);
	...
	static Boolean parseRTSPURL(UsageEnvironment& env, char const* url,
			...
			portNumBits& portNum, char const** urlSuffix = NULL);
	...
	void disallowBasicAuthentication() {
		fAllowBasicAuthentication = False;
	}
...
private:
	...
	Boolean handlePLAYResponse(MediaSession& session,
			...
			const char* speedParamsStr, char const* rangeParamsStr,
...
public:
	static HandlerServerForREGISTERCommand* createNew(UsageEnvironment& env,
			onRTSPClientCreationFunc* creationFunc, Port ourPort = 0,
...
protected:
	HandlerServerForREGISTERCommand(UsageEnvironment& env,
			onRTSPClientCreationFunc* creationFunc, int ourSocket, Port ourPort,
