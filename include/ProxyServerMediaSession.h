...
public:
	ProxyRTSPClient(class ProxyServerMediaSession& ourServerMediaSession,
			...
			int socketNumToServer, unsigned interPacketGapMaxTime = 0,
...
private:
	...
	static void sendLivenessCommand(void* clientData, CommonPlay *cpObj);
	void checkInterPacketGaps_(Boolean delayReset);
	static void checkInterPacketGaps(void* clientData, CommonPlay *cpObj);
	void doReset();
	static void doReset(void* clientData, CommonPlay *cpObj);

	void scheduleDESCRIBECommand();
	static void sendDESCRIBE(void* clientData, CommonPlay *cpObj);

	static void subsessionTimeout(void* clientData, CommonPlay *cpObj);
...
private:
	...
	TaskToken fLivenessCommandTask, fDESCRIBECommandTask, fSubsessionTimerTask,
			fResetTask, fInterPacketGapsTask;
};
...
protected:
	ProxyServerMediaSession(UsageEnvironment& env, CommonPlay *cpObj,
			...
			createNewProxyRTSPClientFunc* ourCreateNewProxyRTSPClientFunc =
					defaultCreateNewProxyRTSPClientFunc,
			portNumBits initialPortNum = 6970, Boolean multiplexRTCPWithRTP =
					False);
...
public:
	PresentationTimeSessionNormalizer(UsageEnvironment& env, CommonPlay *cpObj);
...
private:
	// called only from within "~PresentationTimeSubsessionNormalizer":
