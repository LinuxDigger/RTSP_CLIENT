...
#ifndef INCLUDE_QUICKTIMEFILESINK_H_
#define INCLUDE_QUICKTIMEFILESINK_H_
...
public:
	static QuickTimeFileSink* createNew(UsageEnvironment& env,
			...
			Boolean packetLossCompensate = False, Boolean syncStreams = False,
			Boolean generateHintTracks = False, Boolean generateMP4Format =
					False);
...
#define _atom(name) unsigned addAtom_##name()
	...
	_atom(moov);_atom(mvhd);_atom(iods); // for MP4 format files
	_atom(trak);_atom(tkhd);_atom(edts);_atom(elst);_atom(tref);_atom(hint);_atom(mdia);_atom(mdhd);_atom(hdlr);_atom(minf);_atom(smhd);_atom(vmhd);_atom(gmhd);_atom(gmin);
	unsigned addAtom_hdlr2();_atom(dinf);_atom(dref);_atom(alis);_atom(stbl);_atom(stsd);
	unsigned addAtom_genericMedia();
	unsigned addAtom_soundMediaGeneral();_atom(ulaw);_atom(alaw);_atom(Qclp);_atom(wave);_atom(frma);_atom(Fclp);_atom(Hclp);_atom(mp4a);
//                            _atom(wave);
//                                _atom(frma);
...
#endif /* INCLUDE_QUICKTIMEFILESINK_H_ */
